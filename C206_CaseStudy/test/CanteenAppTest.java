...
public class CanteenAppTest {
	...
	public void setUp() throws Exception {
		...
		// Purchase Order -> sajnam
		purchaseList = new ArrayList<PurchaseOrder>();
		po1 = new PurchaseOrder("", 2, 2.20, "2020-9-20");
		po2 = new PurchaseOrder("carrots", 2, 2.30, "2020-10-7");
		po3 = new PurchaseOrder("onions", 1, 1.30, "2020-2-7");
...
//		PurchaseOrder po1 = new PurchaseOrder("", 2, 2.20, "2020-9-20");
		...
		assertEquals("Test that Purchase Order Item arraylist size is 1", 1, purchaseList.size());
		assertSame("Test that Purchase Order Item is added", po1, purchaseList.get(0));
		// normal
		purchaseList.add(po2);
		assertEquals("Test that Purchase Order Item arraylist size is 2", 2, purchaseList.size());
		assertSame("Test that Purchase Order Item is added", po2, purchaseList.get(1));

		// given adding 2 items in empty list, size of list is 2.
		assertEquals("Test that purchase order arraylist size is 2", 2, purchaseList.size());
	}
	...
	public void deletePurchaseOrderTest() {
		...
		purchaseList.add(po2);
		// boundary condition
		assertNotNull("Test if there is valid PurchaseOrder arraylist to add to", purchaseList);
		// normal condition
		Boolean ok = purchaseList.add(po3);
		assertTrue("Test if the existing Purchase Order item can be deleted from the list", ok);
		// error condition
		ok = purchaseList.add(po4);
		assertTrue("Test if the existing purchase Order item can be deleted from the list?", ok);
	}

	@Test
	public void viewPurchaseOrderTest() {

		// given adding 2 items in the list, there should be 2 items shown
		purchaseList.add(po1);
		purchaseList.add(po2);
		assertEquals("Test that purchase order arraylist size is 2", 2, purchaseList.size());

		// purchase order exists, can be view
		purchaseList.add(po1);
//		CanteenApp.searchPOitem(purchaseList); //This got issue,
		assertNotNull("Test if the purchase order is in the list?", po1);

		// purchase order does not exists, cannot be viewed
//		CanteenApp.searchPOitem(purchaseList); //This got issue,
		assertNotNull("Test if the purchase order is not in the list?", po2);// Need change,
...
//---------------------------------------------------------------------------
	...
	public void addPromotionTest() {
		...
		assertTrue(true);
		// Test that the input has "Promo" as the start and the endDate is before the
		// expire date - normal
		...
		for (int i = 0; i < promotionList.size(); i++) {
			if (promotionList.get(i).getPromotionCode().equals("Promo1")
					&& promotionList.get(i).getEndDate().equals("2020-08-31")) {
				assertFalse(false);
			}
		}
	...
	public void deletePromotionTest() {
		...
		// Check that the promotion is in promotionList then remove the promotion -
		// normal
		ArrayList<Promotion> promotionList = new ArrayList<Promotion>();
		promotionList.add(new Promotion("Promo1", "2020-08-31", 1));

		boolean check = false;
		if (promotionList.get(0).getPromotionCode().equalsIgnoreCase("Promo1")
				&& promotionList.get(0).getEndDate().equalsIgnoreCase("2020-08-31")) {
